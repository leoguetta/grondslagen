...
\noindent\textbf{Exercise 1.} 
  ...
  \item (2 points) Let $P$ and $Q$ be posets and $f \colon P \to Q$ a
    function. Prove that if $P$ is totally ordered, then the following
...
\end{enumerate}
Given an element $x \in T$, an \emph{immediate successor} of $T$ is an
...
element $x \in T$, the set of immediate successor of $x$ is finite.
\begin{enumerate}
\item (3 points) Let $(T,\leq)$ be an infinite tree which is locally
  finite. Prove that there is an immediate successor $x$ of the root
  such that the set $\{y \in T\,\vert\, y \geq x\}$ is infinite.
\item (7 points) Prove the famous K\H{o}nig's lemma: For every infinite and locally
  ...
  \[
    x_0 < x_1 < x_2 < \dots.
  \]
  (Hint: one form or another of the axiom of choice has to be used.)
\end{enumerate}
\noindent\textbf{Exercise 3.} Consider the language $L=\{f\}$, with $f$
...
\end{enumerate}
% \noindent\textbf{Exercise 5.} We consider the language $L_{\R}$ of
% $\R$-vector spaces: it has one constant $0$, a binary function symbol $+$ for
% the addition of vectors, and for each $r\in \R$, a unary function symbol
% $f_r$ for the multiplication of a vector with the scalar $r$. We
% consider also the theory $T_{\R}$ of $\R$-vector spaces, given by the
% axioms
% \[
  
% \]
% Prove with the help of the compactness theorem that there is no
% $L_{\R}$-formula $\phi(x,y)$ with two free varibales $x$ and $y$, that
% expresses that $x$ and $y$ are linearly independant (problem)
\end{document}
