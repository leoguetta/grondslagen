...
\noindent\textbf{Exercise 1.} 
  ...
  injective, and let $x<y$. Since $f$ is injective, $f(x)\neq f(y)$,
  hence $f(x)<f(y)$.

  $(b)\Rightarrow (a)$ Suppose that $f$ is strictly
  increasing. Clearly $f$ is order-preserving, hence we have to prove
  that $f$ is injective. Let $x,y \in X$ such that $f(x)=f(y)$. Since
  $X$ is linearly ordered, we either have $x>y$ or $x>y$ or
  $x=y$. Using that $f$ is strictly increasing, we see that the only
  possibility is $x=y$.
  
  $(b)\Rightarrow (c)$: Suppose that $f$ is strictly increasing and
  let $x,y \in X$ such that $f(x)<f(y)$. Suppose by contradiction that
  $x \nless y$. Because $X$ is linearly ordered, this means that necessarily $x
  ...
  \item Let $A \subseteq L$ be the set of those $x$ such that
    ...
    \begin{itemize}
      \item if $x$ is a successor, $x=y+1$. Because $g$ is strictly
        increasing, we have $g(y)<g(x)$, and because $f$ is an
        embedding, we have $f(x)=f(y)+1$. By hypothesis, $y \in A$,
        hence $f(y)\leq g(y)$. This implies that $f(y)<g(x)$, hence
        \[
          f(x)=f(y)+1\leq g(x).
        \]
    ...
    \item The ``only if'' part is trivial, since, by definition, an
      ...
      L_{<x} \ to M$, we need to give a rule to define $f(x)$. There
      are two cases:
      \begin{itemize}
...
\item Let us first prove the following fact: if $x<y$, then $x'\leq y$
  ...
  \item We define $x_0$ to be the root, and if the sequence has be
    defined up to $x_n$ for some $n \in \N$, we define $x_{n+1}$ to
