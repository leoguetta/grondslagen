...
\item Let us first prove the following fact: if $x<y$, then $x'\leq y$
  ...
  \item Notice that if such a sequence exist, then for every $x_n$,
...
\begin{enumerate}
  \item We proceed by induction on the cardinality $n$ of $X$. If
    ...
    have $A
    \subseteq X-\{x\}$. If $A$ is empty, this means that for
    every element $y \neq x$ of $X$, either $y < x$, or $y$ and $x$
    are not comparable (i.e. neither $x >y$, nor $x < y$). In particular,
    ...
    If $A$ is not empty, then it has a lowest element $x_k$. If $k=0$,
  ...
  \item Consider the language $L_{X}$ which has one binary relation
    ...
    posets $T_{\mathrm{poset}}$ as well as the set of axioms
    \[
      \{c_x \prec c_y \,\vert\, \text{ for all }x< y \in X\}.
    \]
    If we can prove that $T_{X}$ has a model $M$, then the function $X
    \to M, x \mapsto c_x$ is strictly increasing and taking the
    ...
    \[
      T' \subset T_{\mathrm{poset}}\cup \{c_x \prec c_y \,\vert\,
      \text{ for a finite set of }x<y \text{ in } X\}.
    \]
